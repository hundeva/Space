...
class PopularFragmentPresenter extends FragmentPresenter<PopularFragment> {
    ...
    @Inject
    public PopularFragmentPresenter(NasaImagesRepository repository, NasaImagesStore store, NasaMediaAdapter adapter) {
    ...
    @Override
    public void attach(PopularFragment fragment) {
    ...
    private void setupViews(PopularFragment fragment) {
    ...
    private void showStoredItems() {
        ImageSearchResponse storedResponse = store.getMostPopularItems();
    ...
    private void refreshItems(boolean force) {
        if (force || observable == null) {
            if (store.hasPopularItemsStored()) fragment.refreshLayout.setRefreshing(true);
            observable = repository.getMostPopularImages();
